...
struct TVector2 {
  ...
  TVector2 operator*(const T t) const { return TVector2(x * t, y * t); }
  ...
  decltype(auto) length2() const { return x * x + y * y; }

  decltype(auto) length() const { return std::sqrt(x * x + y * y); }
...
struct TVector3 {
  ...
  TVector3 operator*(const T t) const { return TVector3(x * t, y * t, z * t); }
  ...
  decltype(auto) length2() const { return x * x + y * y + z * z; }

  decltype(auto) length() const { return std::sqrt(x * x + y * y + z * z); }
...
TVector3<T> cross(const TVector3<T> &v1, const TVector3<T> &v2) {
  return TVector3<T>(v1.y * v2.z - v1.z * v2.y, v1.z * v2.x - v1.x * v2.z,
                     v1.x * v2.y - v1.y * v2.x);
}
