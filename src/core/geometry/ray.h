...
struct TRay3 {
 public:
  ...
  TRay3(const TPoint3<T> &_ori, const TVector3<T> &_dir,
        std::shared_ptr<Medium> _medium = nullptr, T _time = .0f,
        T _tmin = 0.0005, T _tmax = 1e10)
      : ori(_ori),
        dir(normalize(_dir)),
        time(_time),
        tmin(_tmin),
  ...
  TRay3(const TPoint3<T> &_ori, const TPoint3<T> &_end,
        std::shared_ptr<Medium> _medium = nullptr, T _time = .0f)
      : ori(_ori), time(_time), medium(_medium.get()) {
  ...
  TPoint3<T> at(T dist) const { return ori + dist * dir; }
  ...
  bool is_ray_differential =
      false;  // set to true when camera generate ray-differential
...
struct RayIntersectionRec {
  ...
  void clear() { *this = RayIntersectionRec(); }
