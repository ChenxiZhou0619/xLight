...
void BSDF::bumpComputeShadingNormal(RayIntersectionRec *i_rec) const {
  ...
  float du = i_rec->dudx + i_rec->dudy, dv = i_rec->dvdx + i_rec->dvdy;

  float df_du = m_bumpmap->dfdu(i_rec->UV, du, dv)[0],
        df_dv = m_bumpmap->dfdv(i_rec->UV, du, dv)[0];

  i_rec->shdN = Normal3f(n + df_du * cross(n, i_rec->dpdv) -
                         df_dv * cross(n, i_rec->dpdu));
...
void BSDF::computeShadingNormal(RayIntersectionRec *i_rec) const {
  ...
  Normal3f normal{normal_value.r() * 2 - 1, normal_value.b() * 2 - 1,
                  normal_value.g() * 2 - 1};
...
SpectrumRGB BSDF::evaluate(const SurfaceIntersectionInfo &info,
                           Vector3f _wo) const {
  Vector3f wi = info.toLocal(info.wi), wo = info.toLocal(_wo);
...
float BSDF::pdf(const SurfaceIntersectionInfo &info, Vector3f _wo) const {
  Vector3f wi = info.toLocal(info.wi), wo = info.toLocal(_wo);
  BSDFQueryRecord bRec;
  bRec.uv = info.uv;
  bRec.wi = wi;
...
float BSDF::pdf(const SurfaceIntersectionInfo &info, Vector3f wi,
                Vector3f wo) const {
  wi = info.toLocal(wi), wo = info.toLocal(wo);
  BSDFQueryRecord bRec;
  bRec.uv = info.uv;
  bRec.wi = wi;
  bRec.wo = wo;
