...
struct SurfaceIntersectionInfo {
    ...
    Ray3f generateShadowRay(const Scene &scene, Point3f destination) const;
...
struct MediumIntersectionInfo {
    ...
    //* Distance between the hitpoint and ray.ori
    float       distance;
    //* Direction of the wi (backwards camera)
    Vector3f    wi;
    //* The local shading frame of the hitpoint
    Frame       shadingFrame;
...
struct IntersectionInfo {
    ...
    operator bool() const {
        auto sIts = asSurfaceIntersection();
        if (sIts && sIts->shape == nullptr) return false;
        return true;
    }
