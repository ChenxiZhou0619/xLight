...
class ImageBlock {
 public:
  ...
  ~ImageBlock() {
    for (int i = 0; i < size.x; ++i) delete[] pixels[i];
    delete[] pixels;
  }

  ImageBlock(const Vector2i &_offset, const Vector2i &_size)
      : offset(_offset), size(_size) {
    pixels = new SpectrumRGB *[size.x];
    for (int i = 0; i < size.x; ++i) pixels[i] = new SpectrumRGB[size.y];
  }
  ...
  void setAll(SpectrumRGB rgb) {
    for (int i = 0; i < size.x; ++i) {
      for (int j = 0; j < size.y; ++j) pixels[i][j] = rgb;
    }
  }
...
class Image {
 public:
  ...
  Image(Vector2i _size, const std::string &_filename, uint32_t _spp,
        unsigned _channels = 3)
      : size(_size),
        filename(_filename),
        spp(_spp),
        channels(_channels),
        screen(Vector2i(0, 0), _size) {}
  ...
  void putBlock(const ImageBlock &block) {
    ...
    for (int y = block.offset.y; y < rightDown.y; ++y)
      for (int x = block.offset.x; x < rightDown.x; ++x) {
        screen.pixels[x][y] =
            block.pixels[x - block.offset.x][y - block.offset.y];
      }
  }
  ...
  uint32_t getSpp() const { return spp; }

  SpectrumRGB at(Vector2i pos) const { return screen.pixels[pos.x][pos.y]; }
...
class ImageBlockManager {
 ...
 public:
  ...
  ImageBlockManager(const Vector2i &imgSize)
      : x(imgSize.x / BLOCKSIZE), y(imgSize.y / BLOCKSIZE) {
    ...
    for (int _x = 0; _x < x; ++_x)
      for (int _y = 0; _y < y; ++_y) {
        blocks[_x][_y] =
            new ImageBlock(Vector2i{_x * BLOCKSIZE, _y * BLOCKSIZE},
