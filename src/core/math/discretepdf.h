...
struct Distribution1D {
  ...
  void append(float pdfValue) { mCdf.emplace_back(mCdf.back() + pdfValue); }

  std::size_t size() const { return mCdf.size() - 1; }
  ...
  std::size_t sample(float sampleValue) const {
    ...
    std::size_t index =
        (std::size_t)std::max((std::ptrdiff_t)0, entry - mCdf.begin() - 1);
...
struct Distribution2D {
  ...
  Distribution2D(int dim_x, int dim_y)
      : m_dim_x(dim_x), m_dim_y(dim_y), m_distribution_x(dim_x) {
    for (int i = 0; i < dim_x; ++i) {
      m_distribution_y.emplace_back(Distribution1D(dim_y));
    }
  }

  void appendAtX(int x, float pdf) { m_distribution_y[x].append(pdf); }
  ...
  float pdf(Vector2i position) const {
    ...
    float pdf1 = m_distribution_x.pdf(x), pdf2 = m_distribution_y[x].pdf(y);
    return pdf1 * pdf2;
  }
...
class Distrib1D {
 public:
  ...
  Distrib1D(int nEntries) : distrib(nEntries) { objects.reserve(nEntries); }
  ...
  void postProcess() { distrib.normalize(); }

  /**
   * @brief Receive a uniform random in [0, 1], return a sample result and its
   * corresponding pdf
   *
   * @param uniform
   * @return std::pair<Object, float>
   */
  std::pair<Object, float> sample(float uniform) const {
    float pdf = .0f;
