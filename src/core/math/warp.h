...
public:
    ...
    /// Uniformly sample a vector on the unit hemisphere around the pole (0,1,0) with respect to solid angles
    ...
    /// Uniformly sample a vector on the unit hemisphere around the pole (0,1,0) with respect to projected solid angles
    static Vector3f squareToCosineHemisphere(const Point2f &sample) {
        float phi = 2 * M_PI * sample[0],
              theta = std::acos(std::sqrt(sample[1]));
        return Vector3f {
