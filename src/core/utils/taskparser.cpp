...
Vector2i RenderTask::getImgSize() const { return image->getSize(); }
uint32_t RenderTask::getSpp() const { return image->getSpp(); }
...
std::unique_ptr<RenderTask> RenderTaskParser::createTask(
  ...
  for (auto itr = document.MemberBegin(); itr != document.MemberEnd(); ++itr) {
    if (strcmp(itr->name.GetString(), "output") == 0) {
      ...
      const std::string &filename = (*itr).value["filename"].GetString();
      const auto &size = (*itr).value["size"].GetArray();
      int spp = (*itr).value["spp"].GetInt();
      task->image = std::make_unique<Image>(
          Vector2i{size[0].GetInt(), size[1].GetInt()}, filename, spp);
      ...
      for (int i = 0; i < textures.Size(); ++i) {
        ...
        const std::string &textureName = texture["name"].GetString();
        const std::string &textureType = texture["type"].GetString();
        task->textures[textureName].reset(static_cast<Texture *>(
      ...
      for (int i = 0; i < bsdfs.Size(); ++i) {
        ...
        const std::string &bsdfName = bsdf["name"].GetString();
        const std::string &bsdfType = bsdf["type"].GetString();
        BSDF *newBsdf =
            static_cast<BSDF *>(ObjectFactory::createInstance(bsdfType, bsdf));
        if (bsdf.HasMember("textureRef")) {
          const std::string &textureRef = bsdf["textureRef"].GetString();
          newBsdf->setTexture(task->getTexture(textureRef));
        } else {
          newBsdf->setTexture(nullptr);
        }

        if (bsdf.HasMember("bumpmapRef")) {
          const std::string &bumpmapRef = bsdf["bumpmapRef"].GetString();
          newBsdf->setBumpmap(task->getTexture(bumpmapRef));
        } else {
          newBsdf->setBumpmap(nullptr);
        }

        if (bsdf.HasMember("normalmapRef")) {
          const std::string &normalmapRef = bsdf["normalmapRef"].GetString();
          newBsdf->setNormalmap(task->getTexture(normalmapRef));
        } else {
      ...
      for (int i = 0; i < mediums.Size(); ++i) {
        ...
        const std::string &mediumType = medium["type"].GetString();
        const std::string &mediumName = medium["name"].GetString();
        Medium *newMedium = static_cast<Medium *>(
      ...
      for (int i = 0; i < entities.Size(); ++i) {
        ...
        tmp.reset(meshLoader.loadFromFile(filepath));
        // configure the bsdf
        ...
        for (int i = 0; i < meshProperties.Size(); ++i) {
          ...
          const std::string &meshName = property["meshName"].GetString();
          ...
          if (property.HasMember("BSDFRef")) {
            const std::string &bsdfRef = property["BSDFRef"].GetString();
            const auto &itr = task->bsdfs.find(bsdfRef);
            if (itr == task->bsdfs.end()) {
              std::cout << "No such a BSDF : \"" << bsdfRef << "\""
                        << std::endl;
              exit(1);
            }
          ...
          if (property.HasMember("emitterRef")) {
            const std::string &emitterRef = property["emitterRef"].GetString();
            const auto &itr = task->emitters.find(emitterRef);
            if (itr == task->emitters.end()) {
              std::cout << "No such an Emitter : \"" << emitterRef << "\""
                        << std::endl;
              exit(1);
            }
          ...
          if (property.HasMember("mediumRef")) {
            const std::string &mediumRef = property["mediumRef"].GetString();
            const auto &itr = task->mediums.find(mediumRef);
            if (itr == task->mediums.end()) {
              std::cout << "No such an medium : \"" << mediumRef << "\""
                        << std::endl;
              exit(1);
            }
          ...
          if (property.HasMember("isTwoSide")) {
            if (property["isTwoSide"].GetBool()) targetMesh->setTwoSide();
          }
        }
      ...
      const std::string &samplerType =
          (*itr).value["sampler"]["type"].GetString();
      task->sampler.reset(static_cast<Sampler *>(
          ObjectFactory::createInstance(samplerType, (*itr).value["sampler"])));
      std::cout << "Configure the sampler : " << samplerType << std::endl;

      const std::string &cameraType =
          (*itr).value["camera"]["type"].GetString();
      task->camera.reset(static_cast<Camera *>(
          ObjectFactory::createInstance(cameraType, (*itr).value["camera"])));
      std::cout << "Configure the camera : " << cameraType << std::endl;

      const std::string &integratorType =
          (*itr).value["integrator"]["type"].GetString();
      task->integrator.reset(
          static_cast<Integrator *>(ObjectFactory::createInstance(
              integratorType, (*itr).value["integrator"])));
      std::cout << "Configure the integrator : " << integratorType << std::endl;
    }
  }

  std::unique_ptr<RenderTask> res{nullptr};
  res.reset(task);
  return res;
}