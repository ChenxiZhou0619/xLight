...
//* Initialize
std::map<std::string, std::shared_ptr<Configurable>>
Params::ref_cache = {};

bool Params::has_name() const {
...
Point3f Params::fetch(const std::string &key, Point3f default_value) const
{
    ...
    return {
        value[key.c_str()].GetArray()[0].GetFloat(),
...
template<>
SpectrumRGB Params::fetch(const std::string &key, SpectrumRGB default_value) const 
{
    if (!value.HasMember(key.c_str())) {
        return default_value;
    }
