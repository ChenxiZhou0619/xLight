...
class Params {
 public:
  ...
  template <typename Type>
  std::shared_ptr<Type> fetch_ref(
      const std::string &key,
      std::shared_ptr<Type> default_ref = nullptr) const {
    // todo
  }
...
inline Point3f getPoint3f(const std::string &name,
                          const rapidjson::Value &_value) {
  return Point3f{_value[name.c_str()].GetArray()[0].GetFloat(),
...
inline Vector3f getVector3f(const std::string &name,
                            const rapidjson::Value &_value) {
  return Vector3f{_value[name.c_str()].GetArray()[0].GetFloat(),
                  _value[name.c_str()].GetArray()[1].GetFloat(),
                  _value[name.c_str()].GetArray()[2].GetFloat()};
}

inline SpectrumRGB getSpectrumRGB(const std::string &name,
                                  const rapidjson::Value &_value) {
  return SpectrumRGB{getVector3f(name, _value)};
}
