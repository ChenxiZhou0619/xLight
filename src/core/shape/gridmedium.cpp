...
std::unordered_map<std::string, std::shared_ptr<ShapeInterface>> loadVdbFile(
  ...
  openvdb::Mat4R m =
      densityGrid->transform().baseMap()->getAffineMap()->getMat4();
  ...
  result[gridName] = std::make_shared<GridMedium>(
      Point3f(min.x(), min.y(), min.z()), Point3f(max.x(), max.y(), max.z()),
...
GridMedium::GridMedium(Point3f pMin, Point3f pMax,
                       std::shared_ptr<Medium> gridMedium)
...
void rtcGridMediumIntersectFunc(const RTCIntersectFunctionNArguments *args) {
  ...
  auto min = gridMedium->mPMin, max = gridMedium->mPMax;

  RTCRayHitN *rayhit = args->rayhit;
  RTCRayN *ray = RTCRayHitN_RayN(rayhit, 1);
  RTCHitN *hit = RTCRayHitN_HitN(rayhit, 1);

  auto ori_x = RTCRayN_org_x(ray, 1, 0), ori_y = RTCRayN_org_y(ray, 1, 0),
       ori_z = RTCRayN_org_z(ray, 1, 0), dir_x = RTCRayN_dir_x(ray, 1, 0),
       dir_y = RTCRayN_dir_y(ray, 1, 0), dir_z = RTCRayN_dir_z(ray, 1, 0);

  auto tnear = RTCRayN_tnear(ray, 1, 0), tfar = RTCRayN_tfar(ray, 1, 0);
  ...
  for (int axis = 0; axis < 3; ++axis) {
    float origin = ori[axis], minVal = min[axis], maxVal = max[axis];
    if (dir[axis] == 0) {
      if (origin < minVal || origin > maxVal) return;
    } else {
      float invDir = 1 / dir[axis], t0 = (minVal - origin) * invDir,
  ...
  if (tnear >= farT || tfar <= nearT) return;
...
void GridMedium::initEmbreeGeometry(RTCDevice device) {
  ...
  rtcSetGeometryBoundsFunction(this->embreeGeometry, rtcGridMediumBoundsFunc,
                               nullptr);
  rtcSetGeometryIntersectFunction(this->embreeGeometry,
                                  rtcGridMediumIntersectFunc);
  rtcSetGeometryOccludedFunction(this->embreeGeometry,
                                 rtcGridMediumOcculudeFunc);
...
Normal3f GridMedium::getHitNormal(int triIdx) const { return Normal3f(); }

Point2f GridMedium::getHitTextureCoordinate(int triIdx, Point2f uv) const {
  return Point2f();
}

void GridMedium::sampleOnSurface(PointQueryRecord *pRec, Point3f sample) const {
  return;
}

Point3f GridMedium::getVertex(int idx) const { return Point3f(); }

Point3ui GridMedium::getFace(int idx) const { return Point3ui(); }

Normal3f GridMedium::getNormal(int idx) const { return Normal3f(); }

Point2f GridMedium::getUV(int idx) const { return Point2f(); }