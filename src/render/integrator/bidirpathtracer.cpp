...
class BidirectionalPathTracer : public Integrator {
 private:
  ...
  struct PathVertex {
    ...
    static PathVertex create_camera(std::shared_ptr<Camera> camera, Ray3f ray,
                                    SpectrumRGB beta);
    static PathVertex create_light(std::shared_ptr<Emitter> light, Ray3f ray,
                                   SpectrumRGB beta, float pdf);
    static PathVertex create_surface(std::shared_ptr<IntersectionInfo> si,
                                     SpectrumRGB beta, float pdf,
  ...
  int random_walk(const Scene &scene, Sampler *sampler, int max_depth,
                  Ray3f ray, TransportMode mode, float pdf_dir,
                  SpectrumRGB beta, std::vector<PathVertex> *subpath) const {
    ...
    while (true) {
      ...
      if (!surface_its->shape) {
        if (mode == TransportMode::Radiance) {
          vertex =
              PathVertex::create_light(surface_its->light, ray, beta, pdf_fwd);
          ++bounces;
        }
        break;
      }
  ...
  // TODO
  int generate_camera_subpath(const Scene &scene, Sampler *sampler,
                              int max_depth, std::shared_ptr<Camera> camera,
                              Point2i pixel, Point2i resolution,
                              std::vector<PathVertex> *camera_subpath) const {
    Ray3f ray = camera->sampleRayDifferential(pixel, resolution,
                                              sampler->getCameraSample());
    camera_subpath->emplace_back(
        PathVertex::create_camera(camera, ray, SpectrumRGB{1}));

    //* pdf position and pdf direction is 1
    return random_walk(scene, sampler, max_depth - 1, ray,
                       TransportMode::Radiance, 1.f, SpectrumRGB{1.f},
                       camera_subpath) +
           1;
  }

  // TODO
  int generate_light_subpath(const Scene &scene, Sampler *sampler,
                             int max_depth, std::shared_ptr<Camera> camera,
                             Point2i pixel,
  ...
  SpectrumRGB connect_subpath(const Scene &scene,
                              std::shared_ptr<Sampler> sampler, int t, int s,
                              const std::vector<PathVertex> &camera_subpath,
                              const std::vector<PathVertex> &light_subpath,
                              std::shared_ptr<Camera> camera,
                              Point2i *pixel_new) const {
    SpectrumRGB L{.0f};
    if (t > 1 && s != 0 &&
        camera_subpath[t - 1].type == PathVertex::VertexType::Light)
      return L;
    ...
    if (s == 0) {
      ...
      if (pt.is_connectible()) {
        LightSourceInfo light_info =
            scene.sampleLightSource(*pt.info, sampler.get());
        sampled =
            PathVertex::create_light(nullptr, Ray3f(), SpectrumRGB(), .0f);
      }
    }
  }
              ...
              for (int i = 0; i < tile_size; ++i)
                for (int j = 0; j < tile_size; ++j) {
                  ...
                  for (int spp = 0; spp < task->getSpp(); ++spp) {
                    ...
                    // For every sample per pixel, two paths should be
                    // constructed
                    std::vector<PathVertex> camera_subpath(mMaxDepth + 2);
                    std::vector<PathVertex> light_subpath(mMaxDepth + 1);

                    int len_camera_subpath = generate_camera_subpath(
                        *scene, sampler.get(), mMaxDepth + 2, camera, p_pixel,
                        task->film_size, &camera_subpath);
                    int len_light_subpath = generate_light_subpath(
                        *scene, sampler.get(), mMaxDepth + 1, camera, p_pixel,
                        &light_subpath);
                    ...
                    for (int t = 1; t <= len_camera_subpath; ++t) {
                      for (int s = 0; s <= len_light_subpath; ++s) {
                        ...
                        if (depth < 0 || depth > mMaxDepth ||
                          ...
                          continue;
                        Point2i pixel_new;
                        SpectrumRGB Lpath = connect_subpath(
                            *scene, sampler, t, s, camera_subpath,
                            light_subpath, camera, &pixel_new);
                        if (t != 1)
                          Li += Lpath;
                        else
                          film.add_splat(pixel_new, Lpath,
                                         1.f);  // TODO the splat weight
                      }
                    }
