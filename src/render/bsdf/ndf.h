...
class NDF {
 public:
  ...
  virtual std::pair<Normal3f, float> sample(Vector3f wi,
...
class GGX : public NDF {
 public:
  ...
  virtual float eval(Normal3f n) const override {
    ...
    float determinant2 =
        std::pow(n.x / mAlphaX, 2) + std::pow(n.z / mAlphaY, 2) + n.y * n.y;
  ...
  virtual std::pair<Normal3f, float> sample(Vector3f wi,
    ...
    Vector3f T1 = lensq > 0 ? Vector3f(-Vh.z, Vh.x, 0) / std::sqrt(lensq)
                            : Vector3f(1, 0, 0);
    ...
    Vector3f Nh = t1 * T1 + t2 * T2 +
                  std::sqrt(std::max(0.f, 1 - t1 * t1 - t2 * t2)) * Vh;
    Vector3f Ne = normalize(
        Vector3f(mAlphaX * Nh.x, std::max(.0f, Nh.y), mAlphaY * Nh.z));
    float pdf =
        1 / (1 + Lambda(wi)) * std::max(0.f, dot(wi, Ne)) * eval(Ne) / wi.y;
    return {Ne, pdf};
  }

  virtual float pdf(Vector3f wi, Vector3f n) const override {
    float pdf =
        1 / (1 + Lambda(wi)) * std::max(0.f, dot(wi, n)) * eval(n) / wi.y;
    return pdf;
  }

  virtual float Lambda(Vector3f v) const override {
    return (std::sqrt(
                1 + (std::pow(v.x * mAlphaX, 2) + std::pow(v.z * mAlphaY, 2)) /
                        (v.y * v.y)) -
            1) *
           0.5f;
  }
