...
class DisneyDiffuse : public BSDF {
 public:
  ...
  virtual SpectrumRGB evaluate(const BSDFQueryRecord &bRec) const override {
    ...
    SpectrumRGB diffuse = m_texture->evaluate(bRec.uv) * INV_PI * fdi * fdo *
    ...
    SpectrumRGB subsurface =
        ...
        (fssi * fsso *
             (1 / (Frame::cosTheta(bRec.wi) + Frame::cosTheta(bRec.wo)) - 0.5) +
         0.5) *
        Frame::cosTheta(bRec.wo);
 ...
 protected:
  ...
  float FresnelDiffuse(float fd90, float cosTheta) const {
    return (1 + (fd90 - 1) * std::pow(1 - cosTheta, 5));
  }

  float FresnelSubserface(float fss90, float cosTheta) const {
    return (1 + (fss90 - 1) * std::pow(1 - cosTheta, 5));
  }
};
...
class DisneyMetal : public BSDF {
 public:
  ...
  virtual SpectrumRGB evaluate(const BSDFQueryRecord &bRec) const override {
    ...
    SpectrumRGB Fresnel = baseColor + SpectrumRGB(1 - baseColor.max()) *
                                          std::pow(1 - dot(half, bRec.wo), 5);
  ...
  virtual SpectrumRGB sample(BSDFQueryRecord &bRec, const Point2f &sample,
    ...
    *type = ScatterSampleType::SurfaceReflection;
    ...
    auto res = Fresnel / (1 + ndf->Lambda(bRec.wo)) * dot(half, bRec.wo) /
               dot(half, bRec.wi);
...
class DisneyClearcoat : public BSDF {
 public:
  ...
  virtual SpectrumRGB evaluate(const BSDFQueryRecord &bRec) const override {
    Vector3f half = normalize(bRec.wi + bRec.wo);
    SpectrumRGB Fresnel =
        SpectrumRGB(R0 + (1 - R0) * std::pow(1 - dot(half, bRec.wo), 5));
    auto D = (mAlphaG * mAlphaG - 1) * INV_PI / std::log(mAlphaG * mAlphaG) /
  ...
  virtual SpectrumRGB sample(BSDFQueryRecord &bRec, const Point2f &sample,
    ...
    *type = ScatterSampleType::SurfaceReflection;
    float cosElevation =
              std::sqrt((1 - std::pow(mAlphaG * mAlphaG, 1 - sample.x)) /
                        (1 - mAlphaG * mAlphaG)),
          sinElevation = std::sqrt(1 - cosElevation * cosElevation);
    float azimuth = 2 * M_PI * sample.y;

    Normal3f half{sinElevation * std::cos(azimuth), cosElevation,
                  sinElevation * std::sin(azimuth)};
    ...
    float pdfHalf = (mAlphaG * mAlphaG - 1) * cosElevation * sinElevation /
                    (mAlphaG * mAlphaG * cosElevation * cosElevation +
 ...
 protected:
  ...
  float G(Vector3f w) const {
    float Sigma =
        std::sqrt((1 + (std::pow(w.x * 0.25, 2) + std::pow(w.z * 0.25, 2)) /
                           (w.y * w.y))) +
        1.f;
    Sigma *= 0.5;
    return 1 / Sigma;
