...
class RoughDielectric : public BSDF {
 public:
  ...
  virtual SpectrumRGB evaluate(const BSDFQueryRecord &bRec) const override {
    ...
    if (is_reflect) {
      return SpectrumRGB{0.25f * F * G * D /
                         std::abs(Frame::cosTheta(bRec.wi))};
    } else {
      float top = std::abs(dot(m, bRec.wi) * dot(m, bRec.wo)) * eta * eta *
                  (1 - F) * G * D;
  ...
  virtual SpectrumRGB sample(BSDFQueryRecord &bRec, const Point2f &sample,
    ...
    if (is_reflect) {
      ...
      *type = ScatterSampleType::SurfaceTransmission;
      ...
      float cos_theta_t = std::sqrt(std::max(
          .0f, 1 - (1 - cos_theta_i * cos_theta_i) * inv_eta * inv_eta));
      if (cos_theta_i > 0) cos_theta_t = -cos_theta_t;
      bRec.wo = normalize(-inv_eta * bRec.wi +
    ...
    if (is_reflect) {
      ...
      pdf = microfacet_pdf * std::abs(eta * eta * dot(m, bRec.wo)) /
            (dot(m, bRec.wi) + eta * dot(m, bRec.wo)) /
            (dot(m, bRec.wi) + eta * dot(m, bRec.wo));
    }
    float G = BeckmannDistribution::G(bRec.wi, bRec.wo, m_alpha);
    return SpectrumRGB{std::abs(dot(m, bRec.wi) / (Frame::cosTheta(bRec.wi) *
                                                   Frame::cosTheta(m))) *
                       G};
  }
