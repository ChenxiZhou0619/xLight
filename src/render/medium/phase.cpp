...
class IsotropicPhase : public PhaseFunction {
 public:
  ...
  virtual SpectrumRGB sample(PhaseQueryRecord *pRec,
...
class HenyeyGreensteinPhase : public PhaseFunction {
 public:
  ...
  virtual SpectrumRGB sample(PhaseQueryRecord *pRec,
    ...
    pRec->wo =
        Vector3f{sinTheta * std::cos(phi), cosTheta, sinTheta * std::sin(phi)};
 ...
 protected:
  float HGPhase(float cosTheta) const {
    ...
    return 0.25f * INV_PI * (1 - mG * mG) / (denom * std::sqrt(denom));
  }
